...
class Encoder(nn.Module):
    def __init__(self, n_feature = 16, in_channel = 6):
        super(Encoder, self).__init__()
        self.n_feature = n_feature
        out_channel = n_feature * 2 * n_feature
        self.W = get_MLP_layers((in_channel, n_feature, n_feature*2, out_channel))
    ...
    def forward(self, X_left, X_right, Feature_left, Feature_right):
        ...
        '''
        input_p_left = torch.cat((X_left, X_right),1)  # (n, 6)
        input_p_right = torch.cat((X_right, X_left),1)  # (n, 6)
        
        input_f_left = torch.cat((Feature_left, Feature_right),1).unsqueeze(2)  # (n, 2*d, 1)
        input_f_right = torch.cat((Feature_right, Feature_left),1).unsqueeze(2)  # (n, 2*d, 1)

        out_left = self.W(input_p_left)  # (n, d*d*2)
        out_right = self.W(input_p_right)  # (n, d*d*2)
        out_left = out_left.view(-1, self.n_feature, 2 * self.n_feature)  # (n, d, d*2)
        out_right = out_right.view(-1, self.n_feature, 2 * self.n_feature)  # (n, d, d*2)
        
        out_b_left = self.b(input_p_left)  # (n, d)
        out_b_right = self.b(input_p_right)  # (n, d)
        out_w_left = torch.matmul(out_left, input_f_left).squeeze(2)  # (n, d)
        out_w_right = torch.matmul(out_right, input_f_right).squeeze(2)  # (n, d)
        
        out_feature_left = torch.add(out_w_left, out_b_left)  # (n, d)
        out_feature_right = torch.add(out_w_right, out_b_right)  # (n, d)
        out_feature = torch.add(out_feature_left, out_feature_right)  # (n, d)
        
        return out_feature
    
class Decoder(nn.Module):
    def __init__(self, n_feature = 16, in_channel = 131):
        super(Decoder, self).__init__()
        self.n_feature = n_feature
        out_channel = (n_feature + 3 + 1) * 2
    ...
    def forward(self, Feature_father, P_father):
        ...
        left_P_s = self.sigmoid(out_father[:,(self.n_feature + 2):(self.n_feature + 3)])  # (n, 1)
        left_P = torch.cat((left_P_xy, left_P_s), 1)  # (n, 3)
        left_isleaf = self.sigmoid(out_father[:,(self.n_feature + 3):(self.n_feature + 4)])  # (n, 1)
        
        right_featrue = out_father[:,(self.n_feature + 4):(self.n_feature * 2 + 4)]  # (n, d)
        right_P_xy = self.tanh(out_father[:,(self.n_feature * 2 + 4):(self.n_feature * 2 + 6)])  # (n, 2)
        right_P_s = self.sigmoid(out_father[:,(self.n_feature * 2 + 6):(self.n_feature * 2 + 7)])  # (n, 1)
        right_P =  torch.cat((right_P_xy, right_P_s), 1)  # (n, 3)
        right_isleaf = self.sigmoid(out_father[:,(self.n_feature * 2 + 7):])  # (n, 1)
...
class AE(SaveableModule):
    def __init__(self, device, weight=1, model_folder='log', save_name='ae', n_feature = 16, encode_in_channel = 6, decode_in_channel = 19):
        ...
        self.n_feature = n_feature
        self.encoder = Encoder(n_feature, encode_in_channel)
        self.decoder = Decoder(n_feature, n_feature+3)
        self.weight = weight
    ...
    def decode(self, X, Node_is_leaf, Feature, I_list):
        '''
        Input:
            X: Positions of ALL Tree Nodes  n*3
        ...
        for i in range(num_I):
            ...
            position_loss = torch.mean(position_loss) * self.weight 
    ...
    def forward(self, X, Feature, I_list, Node_is_leaf):
        ...
        # (n, 2), (n, 2), (n, d), (1), (1)
        X_ab_xy, X_ab_xy_r, Feature, Loss_P, Loss_Leaf, Num = self.decode(X, Node_is_leaf, Feature_New, I_list)
        # print(X_ab_xy, X_ab_xy_r)
        Loss_ab = self.cal_distance_ab(X_ab_xy[:,:2], X_ab_xy_r[:,:2])
        Loss_ab = torch.mean(Loss_ab) * self.weight 
    ...
    def cal_distance2(self, p, q, details = False):
        '''
        Input:
            p, q: position of a node  n*3
        Output:
        ...
        # dis_xy = torch.sqrt(((px - qx) * (px - qx)) + ((py - qy) * (py - qy)))
        dis_s = torch.abs(ps-qs)
        dis = dis_xy + dis_s

        # dis = (((px - qx) * (px - qx)) + ((py - qy) * (py - qy)) + ((ps - qs) * (ps - qs)))/3
        if(details):
            return dis, dis_xy, dis_s
            # return dis, dis, dis
        else:
            return dis

    def cal_distance_ab(self, p, q):
        '''
        Input:
