...
def augment_strokes(strokes, prob=0.0):
  ...
  result = []
  prev_stroke = [0, 0, 0, 0, 0,1]
  count = 0
  stroke = [0, 0, 0, 0, 0,1]  # Added to be safe.
  for i in range(len(strokes)):
    candidate = [strokes[i][0], strokes[i][1], strokes[i][2],strokes[i][3],strokes[i][4],strokes[i][5]]
    if candidate[5] == 1 or prev_stroke[5] == 1:
      count = 0
    else:
      count += 1
    urnd = np.random.rand()  # uniform random variable
    if candidate[5] == 0 and prev_stroke[5] == 0 and count > 2 and urnd < prob:
      stroke[0] += candidate[0]
      stroke[1] += candidate[1]
      stroke[2] += candidate[2]
      stroke[3] += candidate[3]
      stroke[4] += candidate[4]
    else:
      stroke = candidate
      prev_stroke = stroke
...
def to_normal_strokes(big_stroke):
  ...
  if l == 0:
    l = len(big_stroke)
  result = np.zeros((l, 6))
  result[:, 0:5] = big_stroke[0:l, 0:5]
  result[:, 5] = big_stroke[0:l, 6]
  return result
...
def to_big_strokes(stroke, max_len=250):
  ...
  result = np.zeros((max_len, 8), dtype=float)
  l = len(stroke)
  assert l <= max_len
  result[0:l, 0:5] = stroke[:, 0:5]
  result[0:l, 6] = stroke[:, 5]
  result[0:l, 5] = 1 - result[0:l, 6]
  result[l:, 7] = 1
  return result
...
class DataLoader(object):
  ...
  def __init__(self,
    ...
    self.start_stroke_token = [0, 0, 0, 0, 0, 1, 0, 0]  # S_0 in sketch-rnn paper
  ...
  def random_scale(self, data):
    ...
    result[:, 2] *= a_scale_factor
    result[:, 3] *= b_scale_factor
    result[:, 4] *= slope_scale_factor
    return result
  ...
  def calculate_normalizing_scale_factor(self):
    ...
    for i in range(len(self.strokes)):
      ...
      for j in range(len(self.strokes[i])):
        ...
        data.append(self.strokes[i][j, 2])
        data.append(self.strokes[i][j, 3])
        data.append(self.strokes[i][j, 4])
  ...
  def normalize(self, scale_factor=None):
    ...
    for i in range(len(self.strokes)):
      self.strokes[i][:, 0:5] /= self.scale_factor
  ...
  def pad_batch(self, batch, max_len):
    ...
    result = np.zeros((self.batch_size, max_len + 1, 8), dtype=float)
...
#       print(batch[i].shape)
      ...
      result[i, 0:l, 0:5] = batch[i][:, 0:5]
      result[i, 0:l, 6] = batch[i][:, 5]
      result[i, 0:l, 5] = 1 - result[i, 0:l, 6]
      result[i, l:, 7] = 1
      # put in the first token, as described in sketch-rnn methodology
      result[i, 1:, :] = result[i, :-1, :]
      result[i, 0, :] = 0
      result[i, 0, 5] = self.start_stroke_token[5]  # setting S_0 from paper.
      result[i, 0, 6] = self.start_stroke_token[6]
      result[i, 0, 7] = self.start_stroke_token[7]
